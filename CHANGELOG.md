...
- **Register-only delimiter loop**: the scanner's column-end count travelled by `ref` into the inlined append loop, so the JIT kept it in memory and every delimiter paid a stack reload, store and bounds check. A Zen 3 instruction profile showed that dependency as about half of the scanner's time on that microarchitecture. The count now travels by value; on the same EPYC 7763 instance, unquoted 10,000 x 25 reads are 18% faster for UTF-8 and 12% for UTF-16, moving from 1.10-1.16x Sep's time to 0.90-1.01x. A CPU-targeted profiling workflow (`profile-zen3.yml`) with an on-demand same-instance A/B was added to find this.
