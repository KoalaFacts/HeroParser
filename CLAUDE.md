...
8. **Vector pairs in a two-chunk unquoted block on AVX2** (PR #90): same bytes and register footprint as the four-chunk single-vector block, chunks dispatched at 64 elements. Neutral (+1-2%, noise) on an EPYC 9V74 VM running x86-64-v3. Per-chunk overhead is not what limits the unquoted path; its cost is the per-delimiter store loop and the loads, which pairing does not change. The same run showed main already at 0.78-0.84x Sep unquoted on Zen 4 running AVX2 code, while Zen 3 (EPYC 7763) sits at 1.10-1.16x with Sep taking the same absolute time on both: the remaining unquoted gap is Zen 3 microarchitecture specific, not an AVX2 code-path problem, and needs instruction-level profiling on Zen 3 hardware rather than another width experiment.
