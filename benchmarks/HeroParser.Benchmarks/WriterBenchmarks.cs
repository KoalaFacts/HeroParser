...
{
    private WriterBenchmarks.TestRecord[] records = null!;
    ...
    private static WriterBenchmarks.TestRecord[] GenerateRecords(int count)
    {
        var result = new WriterBenchmarks.TestRecord[count];
        for (int i = 0; i < count; i++)
        {
            result[i] = new WriterBenchmarks.TestRecord
            {
                Id = i,
                Name = $"Name{i}",
                Value = i * 1.5,
