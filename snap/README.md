...
2. **Build the Snap**:
   ...
   snapcraft
   ```
   This will produce a `.snap` package (e.g., `heroparser_2.7.0_amd64.snap`).

## Testing the Snap Locally

To install and test the generated snap package locally in strict confinement:
```bash
sudo snap install heroparser_2.7.0_amd64.snap --dangerous
```
...
2. **Upload & Publish**:
   ```bash
   snapcraft upload --release=stable heroparser_2.7.0_amd64.snap
   ```
