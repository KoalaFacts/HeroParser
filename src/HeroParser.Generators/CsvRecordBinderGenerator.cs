...
{
    private const string GENERATED_CHAR_NAMESPACE = "HeroParser.SeparatedValues.Reading.Binding";
    ...
    private const string BINDER_INTERFACE_TYPE = "global::HeroParser.SeparatedValues.Reading.Binders.ICsvBinder<char, ";
...
#pragma warning restore RS2008
    ...
    private static void EmitTypedBinder(SourceProductionContext context, TypeDescriptor descriptor)
    {
        ...
        builder.AppendLine();
        builder.AppendLine($"namespace {GENERATED_CHAR_NAMESPACE};");
        builder.AppendLine();
    ...
    private static void EmitInlinePropertyBinding(SourceBuilder builder, MemberDescriptor member)
    {
        ...
        builder.AppendLine("{");
        builder.Indent();
        builder.AppendLine($"var span = row[{indexField}].Span;");
    ...
    private static void EmitByteDateTimeParsing(SourceBuilder builder, MemberDescriptor member, string parseMethod)
    {
        ...
        if (format != null)
        {
            // Use format string - requires UTF-16 decode
            builder.AppendLine($"if (column.{parseMethod}(out var parsed_{propertyName}, \"{EscapeString(format)}\"))");
        }
        else
        {
            builder.AppendLine($"if (column.{parseMethod}(out var parsed_{propertyName}))");
    ...
    {
        // Register char-based binder (same namespace as registration)
        var binderClassName = $"CsvInlineBinder_{safeClassName}";
        builder.AppendLine($"{BINDER_FACTORY_TYPE}.RegisterCharBinder<{fullyQualifiedName}>({binderClassName}.Create);");

        // Register byte-based binder (different namespace, needs full qualification)
