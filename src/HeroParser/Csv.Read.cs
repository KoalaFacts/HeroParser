...
public static partial class Csv
{
    ...
    /// <summary>
    /// Deserializes CSV data from text into strongly typed records.
    /// Uses UTF-8 byte parsing when a generated byte binder is available; otherwise falls back
    /// to the char-based binder path. For best performance and explicit buffer ownership,
    /// use the byte-based overload.
    /// </summary>
    /// <remarks>
    /// For optimal performance with explicit buffer ownership, use
    /// <see cref="DeserializeRecords{T}(string, out byte[], CsvRecordOptions?, CsvReadOptions?)"/>.
    /// </remarks>
        ...
        where T : new()
    {
        ...
        if (CsvRecordBinderFactory.TryGetByteBinder<T>(recordOptions, out var byteBinder))
        {
            var textBytes = System.Text.Encoding.UTF8.GetBytes(data);
            var byteReader = ReadFromByteSpan(textBytes, parserOptions);
            return CsvRecordReader<char, T>.CreateByteBacked(
                byteReader,
                byteBinder!,
                textBytes,
