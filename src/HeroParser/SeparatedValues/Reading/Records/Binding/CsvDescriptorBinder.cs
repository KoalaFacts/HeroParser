...
public sealed class CsvDescriptorBinder<T> : ICsvBinder<T> where T : class, new()
{
    ...
    private static int FindHeaderIndex(CsvCharSpanRow headerRow, string name, StringComparer comparer)
    {
        var count = headerRow.ColumnCount;
        for (int i = 0; i < count; i++)
        {
            if (comparer.Equals(new string(headerRow[i].CharSpan), name))
                return i;
        }
        return -1;
    }
