...
public sealed class CsvAsyncStreamWriter : IAsyncDisposable
{
    ...
    private readonly Writing.CsvWriterOptions options;
    ...
    private readonly Writing.QuoteStyle quoteStyle;
    ...
    private readonly Writing.CsvInjectionProtection injectionProtection;
    ...
    /// <param name="options">Writer options; defaults to <see cref="Writing.CsvWriterOptions.Default"/>.</param>
    ...
    public CsvAsyncStreamWriter(Stream stream, Writing.CsvWriterOptions? options = null, Encoding? encoding = null, bool leaveOpen = false)
    {
        ArgumentNullException.ThrowIfNull(stream);

        this.stream = stream;
        this.options = options ?? Writing.CsvWriterOptions.Default;
    ...
    private bool TryWriteRowSync(string?[] values)
    {
        ...
        for (int i = 0; i < values.Length; i++)
        {
            ...
            else if (quoteStyle == Writing.QuoteStyle.Always)
            {
    ...
    private bool WriteFieldValueSync(ReadOnlySpan<char> value)
    {
        ...
        // Check for injection
        if (injectionProtection != Writing.CsvInjectionProtection.None && IsDangerousField(value))
        {
        ...
        switch (quoteStyle)
        {
            ...
            case Writing.QuoteStyle.Never:
                ...
                break;
            case Writing.QuoteStyle.WhenNeeded:
            default:
    ...
    private bool WriteSpanFormattableSync<T>(T value, string? format) where T : ISpanFormattable
    {
        ...
        if (injectionProtection != Writing.CsvInjectionProtection.None)
        {
        ...
        if (quoteStyle == Writing.QuoteStyle.Always)
        {
            ...
            var valueSpan = charBuffer.AsSpan(charBufferPosition, charsWritten);
            int quoteCount = CountQuotes(valueSpan);
        ...
        else if (quoteStyle == Writing.QuoteStyle.WhenNeeded)
        {
            // Check if quoting is needed
            var valueSpan = charBuffer.AsSpan(charBufferPosition, charsWritten);
            var (needsQuoting, quoteCount) = AnalyzeFieldForQuoting(valueSpan);
    ...
    private async ValueTask WriteFieldValueAsync(ReadOnlyMemory<char> value, CancellationToken cancellationToken)
    {
        ...
        if (injectionProtection != Writing.CsvInjectionProtection.None && IsDangerousField(valueSpan))
        {
        ...
        {
            case Writing.QuoteStyle.Always:
                await WriteQuotedFieldWithKnownQuoteCountInternalAsync(value, CountQuotes(valueSpan), cancellationToken).ConfigureAwait(false);
                break;
            case Writing.QuoteStyle.Never:
    ...
    private async ValueTask WriteFieldWithInjectionProtectionInternalAsync(ReadOnlyMemory<char> value, CancellationToken cancellationToken)
    {
        ...
        {
            case Writing.CsvInjectionProtection.None:
            ...
            case Writing.CsvInjectionProtection.EscapeWithQuote:
            ...
            case Writing.CsvInjectionProtection.EscapeWithTab:
                // Prefix with tab inside quotes: =SUM(A1) becomes "\t=SUM(A1)"
                await WriteQuotedFieldWithPrefixInternalAsync('\t', value, cancellationToken).ConfigureAwait(false);
                break;

            case Writing.CsvInjectionProtection.Sanitize:
            ...
            case Writing.CsvInjectionProtection.Reject:
                throw new CsvException(
    ...
    private ValueTask WriteQuotedFieldWithPrefixInternalAsync(char prefix, ReadOnlyMemory<char> value, CancellationToken cancellationToken)
    {
        ...
        int quoteCount = CountQuotes(value.Span);
    ...
    private ValueTask WriteSpanFormattableDirectlyAsync<T>(T value, string? format, CancellationToken cancellationToken) where T : ISpanFormattable
    {
        ...
        if (availableSpace >= MAX_STACK_ALLOC_SIZE)
        {
            ...
            if (value.TryFormat(targetSpan, out int charsWritten, format, culture))
            {
                ...
                if (quoteStyle != Writing.QuoteStyle.Always &&
                    injectionProtection == Writing.CsvInjectionProtection.None)
                {
    ...
    private async ValueTask WriteFieldValueFromBufferAsync(int length, CancellationToken cancellationToken)
    {
        ...
        if (quoteStyle == Writing.QuoteStyle.Always)
        {
            ...
            await WriteQuotedFieldWithKnownQuoteCountInternalAsync(temp.AsMemory(), CountQuotes(valueSpan), cancellationToken).ConfigureAwait(false);
        }
        else
        {
            var (needsQuoting, quoteCount) = AnalyzeFieldForQuoting(valueSpan);
